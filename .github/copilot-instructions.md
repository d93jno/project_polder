...
- **Water depth is drawn, not looked up.** The plane sits at `PresentationCoords.water_surface_m`
  ...
  use `Picking.cell_under_ray`. Do not use `world_ground` for those. The rules decide what is wet:
  `BowlMap.is_wet` / `step_at` (GDD 5.8), and step cost, hiding and break's Dry clause read the step
  through `step_at`, never `map.water_step`. `PresentationCoords.in_water` must agree with it; an
  invariant test checks. Pieces in
