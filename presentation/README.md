...
`make run` opens `fight_view.gd`: the scripted-fight street (dry, masonry wall, four squad, three Drifters). Kit meshes come from `PresentationCatalog` via `bowl_draw`. Click a tile to walk (free until contact, then AP). Click a hostile to shoot. Q sets a Watch toward the hovered cell. Space / the phase plaque ends a phase; the enemy then takes legal shots and yields.
