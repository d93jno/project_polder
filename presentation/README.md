...
`make run` opens `fight_view.gd` on the **cutaway fixture** (flooded street + roof, plank rifle Watch, smoke apex). Overlays come only from `overlay_queries.gd` (cones, exposure words, path exposure, stack labels). `PgUp`/`PgDn` cutaway; `F` toggles Falling ↔ Flooded. Click a tile to walk (free until contact, then AP). Click a hostile to shoot. Q sets a Watch toward the hovered cell. Space / the phase plaque ends a phase. The scripted-fight opening returns in 2.6.
