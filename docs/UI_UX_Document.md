...
**Height reads.** With a unit selected, every tile shows its level. The player always knows: is that roof above the water, and can I reach it this phase?
...
- **Water is one plane per bowl.** A step-driven shader handles Flooded, Falling and Mud; height comes from data. A levee map holds two planes.
